...
const DEFAULT_INCOME_CATEGORIES = [
  { nameKey: 'categoryNames.service', type: 'income', color: '#22c55e' },
  { nameKey: 'categoryNames.productSales', type: 'income', color: '#3b82f6' },
  { nameKey: 'categoryNames.affiliateCommission', type: 'income', color: '#8b5cf6' },
  { nameKey: 'categoryNames.other', type: 'income', color: '#6b7280' },
];

const DEFAULT_EXPENSE_CATEGORIES = [
  { nameKey: 'categoryNames.itSoftware', type: 'expense', color: '#ef4444' },
  { nameKey: 'categoryNames.marketingAdvertising', type: 'expense', color: '#f97316' },
  { nameKey: 'categoryNames.officeSupplies', type: 'expense', color: '#eab308' },
  { nameKey: 'categoryNames.travelTransport', type: 'expense', color: '#14b8a6' },
  { nameKey: 'categoryNames.education', type: 'expense', color: '#6366f1' },
  { nameKey: 'categoryNames.phoneInternet', type: 'expense', color: '#ec4899' },
  { nameKey: 'categoryNames.insurance', type: 'expense', color: '#84cc16' },
  { nameKey: 'categoryNames.other', type: 'expense', color: '#6b7280' },
];
  ...
  try {
    ...
    if (catCount.count === 0) {
      ...
      for (const cat of [...DEFAULT_INCOME_CATEGORIES, ...DEFAULT_EXPENSE_CATEGORIES]) {
        insert.run(crypto.randomUUID(), t(cat.nameKey), cat.type, cat.color, userRow.id);
      }
    }
...
async function editTransaction(args: string[]): Promise<void> {
  ...
  try {
    ...
    // Build update query
    const ALLOWED_COLUMNS = new Set(['amount', 'description', 'date']);
    ...
    // Validate that only allowed columns are in the SET clause
