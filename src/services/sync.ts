...
import { getConnectionState, saveConnectionState, isConnected, API_URL } from './connection.js';
...
export async function fullSync(): Promise<{
  ...
  if (!isConnected()) {
    return { transactions: 0, categories: 0, errors: ['not_connected'] };
  }

  const state = getConnectionState();
  if (!state.userId) {
    return { transactions: 0, categories: 0, errors: ['no_user_id'] };
  }
