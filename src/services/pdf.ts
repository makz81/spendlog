...
async function loadPuppeteer(): Promise<PuppeteerNode> {
  try {
    ...
    throw new Error('Puppeteer ist nicht installiert. Fuer PDF-Rechnungen: npm install puppeteer');
  }
}
...
export function prepareInvoiceData(invoice: Invoice, profile: Profile): InvoicePdfData {
  ...
  const tax = profile.isKleinunternehmer ? 0 : subtotal * 0.19;
...
export async function generateInvoicePdf(data: InvoicePdfData): Promise<string> {
  ...
  // Generate PDF with Puppeteer (lazy-loaded)
