...
async function addTransaction(args: Record<string, unknown>, type: 'income' | 'expense'): Promise<unknown> {
  ...
  const categoryResult = input.category
    ? await findOrCreateCategory(input.category, type, userId)
  ...
  const i18nKey = type === 'income' ? 'transactions.incomeSaved' : 'transactions.expenseSaved';
  let message = t(i18nKey, {
    amount: formatCurrency(input.amount),
    description: input.description,
  });
  if (project) message += ` (${project.name})`;
...
export async function updateTransaction(args: Record<string, unknown>): Promise<unknown> {
  ...
  if (input.project !== undefined) {
    ...
    if (input.project === '') {
      ...
      // TypeORM needs null (not undefined) to clear a column in SQLite
