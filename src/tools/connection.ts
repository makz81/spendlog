...
export async function connect(): Promise<unknown> {
  ...
  try {
    ...
    saveConnectionState(pendingState as any);
...
export async function connectionStatus(): Promise<unknown> {
  ...
  const pendingState = state as any;
  if (pendingState.pendingToken) {
    // Poll API to check if connection was completed
    const pollResult = await pollConnectionStatus(pendingState.pendingToken, pendingState.deviceId);
    ...
    if (pollResult.connected) {
      ...
      return {
        ...
        device_id: pendingState.deviceId,
        dashboard_url: getDeeplink('dashboard'),
        hint: t('connection.syncHint'),
      };
    }

    const expires = new Date(pendingState.pendingExpires);
    ...
    return {
      ...
      expires_at: pendingState.pendingExpires,
...
async function pollConnectionStatus(
  ...
  try {
    const apiUrl = getApiUrl();
    const response = await fetch(`${apiUrl}/auth/link/status/${token}`);
...
// ============================================================================
// Validate Connection (called by sync service when web completes auth)
// ============================================================================
